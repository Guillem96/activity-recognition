...
def train(model: ar.checkpoint.SerializableModule, train_dl: data.DataLoader,
          valid_dl: data.DataLoader, train_from: dict,
          summary_writer: ar.typing.TensorBoard, **kwargs: Any) -> None:
    """
    Trains the models along specified epochs with the given train and validation
    dataloader.
    """
    criterion_fn = torch.nn.NLLLoss()
    optimizer, scheduler = load_optimizer(model, train_from, len(train_dl),
                                          **kwargs)
    ...
    if summary_writer is not None:
        hparams = {
            **model.config(), 'optimizer': kwargs['optimizer'],
...
def load_optimizer(model: torch.nn.Module,
                   ...
                   **kwargs: Any) -> Tuple[Optimizer, Optional[Scheduler]]:
    """
...
def _load_model(model_name: str,
    ...
    model_classes: Dict[str, Type[ar.checkpoint.SerializableModule]] = dict(
        LRCN=ar.video.LRCN, LRCNWithAudio=ar.video.LRCNWithAudio)
    ...
    if resume_checkpoint is None:
        ...
        model, checkpoint = model_cls.load(resume_checkpoint,
                                           map_location=device,
                                           freeze_feature_extractor=freeze_fe)
...
@click.option('--audio/--no-audio',
              default=False,
              help='Use audio for training too')
# Training behavior
